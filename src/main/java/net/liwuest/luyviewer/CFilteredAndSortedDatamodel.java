...
//  public final class DataFiltered implements CEventBus.AbstractEvent {}
  ...
  private final LinkedHashMap<CMetamodel.Feature, SortOrder> m_Orderings = new LinkedHashMap<>();
  private final Set<CMetamodel.Feature> m_HiddenFeatures = new HashSet<>();
  ...
  public static boolean isSortable(CMetamodel.Feature Feature) { return (false == Feature.multiple) && !Feature.isRelationshipFeature; }
   ...
   */
  public synchronized CFilteredAndSortedDatamodel sort(CMetamodel.Feature Feature, SortOrder Order) {
    if ((null != Feature) && (null != Order)) {
      m_Orderings.remove(Feature);
      if ((SortOrder.UNSORTED != Order) && isSortable(Feature)) {
        m_Orderings.putLast(Feature, Order);
  ...
  public synchronized CFilteredAndSortedDatamodel showFeature(CMetamodel.Feature Feature) {
    if (null != Feature) {
      if (m_HiddenFeatures.remove(Feature)) CEventBus.publish(new FeatureUnhidden(Feature));
    }
    return this;
  }
  ...
  public synchronized LinkedHashSet<CMetamodel.Feature> getOrderedFeatures(CMetamodel.TypeExpression Type) {
    ...
    return Type.features.stream().filter(f -> !m_HiddenFeatures.contains(f.persistentName)).collect(Collectors.toCollection(LinkedHashSet::new));
  }
  ...
  public synchronized LinkedHashSet<? extends CDatamodel.Element> getFilteredAndSortedData(CMetamodel.TypeExpression Type) {
    ...
    LinkedHashSet<CMetamodel.Feature> filteredFeatures = getOrderedFeatures(Type);
    ...
    for (Map.Entry<CMetamodel.Feature, SortOrder> e : m_Orderings.entrySet()) {
      if (filteredFeatures.contains(e.getKey())) {
        result = result.stream().sorted((bb1, bb2) -> {
          int comparisonResult = 0;
          // Try a natural comparison of scalars
          Object o1 = bb1.AdditionalData.getOrDefault(e.getKey().name, null);
          Object o2 = bb2.AdditionalData.getOrDefault(e.getKey().name, null);
          if ((o1 instanceof Comparable c1) && (o2 instanceof Comparable c2)) comparisonResult = c1.compareTo(c2);
            ...
            if ((e.getKey().isEnumerationAttribute) && (o1 instanceof List l1) && (o2 instanceof List l2)) {
              ...
              if ((0 == comparisonResult) && (0 < l1.size())) {
                ...
                if ((l1.get(0) instanceof Comparable c1) && (l2.get(0) instanceof Comparable c2)) comparisonResult = c1.compareTo(c2);
              }
            }
          }
